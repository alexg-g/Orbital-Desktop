...
export type ChatsTabProps = Readonly<{
  ...
  renderThreadDetail: (props: { threadId: string }) => JSX.Element;
  ...
  return (
    <>
      ...
      <div className="Inbox__conversation-stack">
        <div id="toast" />
        {/* Orbital: Show thread detail when in Threads mode with a selected thread */}
        {displayMode === DisplayMode.Threads && selectedThreadId ? (
          <div
            key={selectedThreadId}
            className="Inbox__conversation"
            id={`thread-${selectedThreadId}`}
          >
            {renderThreadDetail({ threadId: selectedThreadId })}
          </div>
        ) : selectedConversationId ? (
          <div
